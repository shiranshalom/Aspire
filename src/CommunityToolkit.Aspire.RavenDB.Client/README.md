...
Install the `Aspire.RavenDB.Client` library with [NuGet](https://www.nuget.org):  
```dotnetcli
dotnet add package Aspire.RavenDB.Client
```
...
The `Aspire.RavenDB.Client` library provides multiple options to configure the database connection to suit your project's needs.
...
For applications where you need a RavenDB container to run alongside your application, consider using the `Aspire.Hosting.RavenDB` library. This is particularly useful for local development or when you want to manage the RavenDB server lifecycle via code.

### Install the Aspire.Hosting.RavenDB Library

Install the `Aspire.Hosting.RavenDB` library with [NuGet](https://www.nuget.org):
```dotnetcli
